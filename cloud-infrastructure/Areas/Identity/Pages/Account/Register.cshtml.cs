...
namespace cloud_infrastructure.Areas.Identity.Pages.Account
{
    public class RegisterModel : PageModel
    {
        ...
        private readonly IUserStore<Developer> _userStore;
        private readonly IUserEmailStore<Developer> _emailStore;
        private readonly ILogger<RegisterModel> _logger;

        public RegisterModel(
            UserManager<Developer> userManager,
            IUserStore<Developer> userStore,
        ...
        private Developer CreateUser()
        {
            try
            {
                return Activator.CreateInstance<Developer>();
            }
            catch
            {
                throw new InvalidOperationException($"Can't create an instance of '{nameof(Developer)}'. " +
                    $"Ensure that '{nameof(Developer)}' is not an abstract class and has a parameterless constructor, or alternatively " +
                    $"override the register page in /Areas/Identity/Pages/Account/Register.cshtml");
            }
        }

        private IUserEmailStore<Developer> GetEmailStore()
        {
            if (!_userManager.SupportsUserEmail)
            {
                throw new NotSupportedException("The default UI requires a user store with email support.");
            }
            return (IUserEmailStore<Developer>)_userStore;
        }
    }
}
